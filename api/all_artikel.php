<?php
    ...
    //$sql = "SELECT artikel.id_artikel, id_kategori, id_topik, id_komoditas, judul_artikel, tanggal, id_gambar, path FROM artikel INNER JOIN tabel_gambar WHERE id_kategori=1 and artikel.id_artikel=tabel_gambar.id_artikel ORDER BY id_artikel DESC LIMIT 4 ";

    $sql = "SELECT id_artikel, id_kategori, id_topik, id_komoditas, judul_artikel, isi_artikel, tanggal, nama_user_input, total_baca, total_komentar FROM artikel_view WHERE id_kategori=1 AND is_remove='N' ORDER BY id_artikel DESC LIMIT 5";
